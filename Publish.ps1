...
param (
    ...
    ## => User Config <= ##
    $ProjectPath = "src/fftivc.katanareforged/fftivc.katanareforged.csproj",
    $PackageName = "ffttic.katanareforged",
    $PublishOutputDir = "Publish/ToUpload",

    ## => User: Delta Config
    # Pick one and configure settings below. Deltas are an update-size optimisation and are OFF for now
    # (the full package updates fine); enable UseGitHubDelta + MakeDelta later once there is a prior
    # release on GitHub to diff against.
    $MetadataFileName = "ffttic.katanareforged.ReleaseMetadata.json",
    $UseGitHubDelta = $False, # GitHub Releases
    $UseGameBananaDelta = $False,
    $UseNuGetDelta = $False,

    $GitHubUserName = "mpelos", # Name of the GitHub user where the mod is contained
    $GitHubRepoName = "FFTKatanaReforged", # Name of the GitHub repo where the mod is contained
    ...
    $GameBananaItemId = 0, # From mod page URL (unused: GameBanana publishing disabled below).

    $NuGetPackageId = "ffttic.katanareforged",
    $NuGetFeedUrl = "http://packages.sewer56.moe:5000/v3/index.json",
    $NuGetAllowUnlisted = $False,

    ## => User: Publish Config
    $PublishGeneric    = $True,  # The generic .zip for GitHub Releases / NexusMods
    $PublishNuGet      = $False, # No NuGet feed for this mod
